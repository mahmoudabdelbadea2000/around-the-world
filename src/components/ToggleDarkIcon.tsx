...
const activeCircle = "flex justify-center items-center w-5 h-5 rounded-full";
type theme = "light" | "dark";
export function ToggleDarkIcon() {
  ...
  useEffect(() => {
    if (localStorage.getItem("theme")) {
      toggleTheme(localStorage.getItem("theme") as theme);
    }
  }, []);
  ...
  return (
    <div className="flex h-8 w-14 items-center justify-around rounded-full bg-[#2e2f33]">
      <button
        className={`${activeCircle} ${theme === "light" && "bg-white"}`}
      ...
      <button
        className={`${activeCircle} ${theme === "dark" && "bg-white"}`}
